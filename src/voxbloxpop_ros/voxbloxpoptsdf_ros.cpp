...
{
  ros::init(argc, argv, "voxbloxtsdf_ros");
  ...
  voxpopros::migrateParamsToArgs(args, data_source->nodeHandle(), voxpopros::paramToArgMappingsTsdf());
  ...
  voxbloxpop::VoxbloxPopTsdf populator(data_source);
  ...
  data_source->setPublishCloudFunction([&populator, &cloud_publisher, &voxel_publisher, data_source]() {
    ...
    if (populator.map() && populator.map()->getTsdfLayerPtr())
    {
      auto &map = *populator.map();
      const auto map_frame = data_source->options().map_frame;
      const float surface_distance_thresh = populator.options().map().surface_distance_threshold;
      ...
      const voxbloxpop::ShouldVisualizeVoxelColorFunctionType<voxblox::TsdfVoxel> visualise_voxel =
        [surface_distance_thresh, preferred_colour](const voxblox::TsdfVoxel &voxel, const voxblox::Point &point,
                                                    voxblox::Color *colour) {
          return voxbloxpop::shouldVisualiseTsdf(voxel, point, surface_distance_thresh, colour, &preferred_colour);
        };
      ...
      map.getTsdfLayerPtr()->removeDistantBlocks(
      ...
      voxpopros::publishMapCloud(cloud_publisher, map_frame, map.getTsdfLayer(), visualise_voxel,
                                 ohmdataros::colourByIntensity());
      voxpopros::publishMapVoxels(voxel_publisher, map_frame, map.getTsdfLayer(), visualise_voxel, "voxels",
                                  colour_solver);
    }
  });
